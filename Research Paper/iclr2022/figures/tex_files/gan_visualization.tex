\begin{tikzpicture}[box/.style={draw,thick,minimum width=5em,minimum
  ...
  \begin{scope}[start chain=A going right,oc/.style={join=by arj,on chain},
   ...
   \node(color data)[outer sep=0pt,inner sep=0pt,label=Ground Truth] at (1.2,1) {\includegraphics[width=2cm]{figures/tex_files/ground_truth.jpg}};
   ...
   \node(input image)[outer sep=0pt,inner sep=0pt,label=Grayscale Image] at (-5.5,-2) {\includegraphics[width=2cm]{figures/tex_files/input_grayscale.jpg}};
   \draw[->] (input image) -- (generator);
   
   \node(generator func)[outer sep=0pt,inner sep=0pt,label=Generated Image] at (1.2,-2){\includegraphics[width=2cm]{figures/tex_files/generated_output.jpg}};
   \draw[<-] (generator func) -- (generator);
   \draw[->] (generator func) -- (discriminator);
   
   %\node(discriminator func)[box, minimum height = 2em, minimum width %= 2em] at (8, -2){\textit{\scriptsize{p}}};
   ...
   \draw[->] (discriminator) -- (8.5,-2) node[above left=-0.1em]{\textit{p}\scriptsize{(prob)}} -- (cost);
